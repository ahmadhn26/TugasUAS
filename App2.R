...
# 2. Muat data spasial (dari peta_sovi_sederhana.gpkg)
spatial_result <- load_spatial_data()
spatial_data_geom <- spatial_result$spatial

# --- LANGKAH PENGGABUNGAN CERDAS ---

# 3. Dari data spasial, kita HANYA ambil kolom kunci dan geometri
spatial_data_geom_clean <- spatial_data_geom %>%
  select(DISTRICTCODE, geom)

# 4. Pastikan tipe data kolom kunci sama
...
# 5. Gabungkan geometri yang bersih dengan SEMUA atribut dari sovi_data
#    Hasilnya adalah satu dataset `spatial_data` yang bersih tanpa duplikasi kolom
spatial_data <- left_join(spatial_data_geom_clean, sovi_data, by = "DISTRICTCODE")

# --- AKHIR DARI PENGGABUNGAN ---

# Buat daftar pilihan peta dari data yang SUDAH digabung (sekarang namanya sudah benar)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_moran, {
    ...
    # Langkah 1: Pastikan input dari dropdown tidak kosong
    req(input$map_var)
    
    # Langkah 2 (Opsional, untuk debugging): Cetak variabel yang dipilih ke console
    print(paste("Menjalankan Moran's I untuk variabel:", input$map_var))
    
    # Langkah 3: Ambil data dari kolom yang dipilih
    # Tambahkan 'try' untuk menangani jika kolom tiba-tiba tidak ada
    var_data <- try(spatial_data[[input$map_var]], silent = TRUE)
    
    # Langkah 4: Pastikan data berhasil diambil sebelum melanjutkan
    if (inherits(var_data, "try-error") || is.null(var_data)) {
      output$moran_test_result <- renderPrint({ "Error: Kolom data tidak ditemukan." })
      return()
    }
    
    # Langkah 5: Lanjutkan dengan analisis jika semua aman
    # Menghilangkan nilai NA (penting untuk uji Moran's I)
    if (any(is.na(var_data))) {
      var_data[is.na(var_data)] <- mean(var_data, na.rm = TRUE)
    }
    
    # Jalankan Uji Moran's I
    moran_result <- moran.test(var_data, listw = spatial_weights, na.action = na.pass)
    
    # Tampilkan hasil mentahnya
    ...
    output$moran_interpretation <- renderText({
      ...
      if (p_val < 0.05) {
        if (moran_I > 0) {
          paste("Hasilnya signifikan (p < 0.05). Nilai Moran's I positif (", round(moran_I, 3), ") menunjukkan adanya pola MENGELOMPOK (clustered). Wilayah dengan nilai serupa cenderung berdekatan.")
        } else {
          paste("Hasilnya signifikan (p < 0.05). Nilai Moran's I negatif (", round(moran_I, 3), ") menunjukkan adanya pola MENYEBAR (dispersed). Wilayah dengan nilai serupa cenderung berjauhan.")
        }
      } else {
        paste("Hasilnya tidak signifikan (p >= 0.05). Tidak ada cukup bukti untuk menyatakan adanya pola spasial (pola data cenderung acak).")
      }
    })
    
  }) # Akhir dari observeEvent
}
